...
Install_Apache_24()
{
	...
	cd ${Setup_Path}/src
	./configure --prefix=${Setup_Path} --enable-mods-shared=most --enable-headers --enable-mime-magic --enable-proxy --enable-so --enable-rewrite --enable-ssl --enable-deflate --with-pcre --with-included-apr --with-apr-util --enable-mpms-shared=all --enable-nonportable-atomics=yes --enable-remoteip --enable-http2 ${ENABLE_HTTP2_PATH} ${ENABLE_SSL_PATH} ${ENABLE_LUA} ${i_make_args}
	make -j${cpuCore}
