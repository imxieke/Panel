...
sysCheck=$(uname -m)
if [ "${ubuntuVerCheck}" == "${debianVerCheck}" ] || [ "${sysCheck}" != "x86_64" ];then
	wget -O lib.sh ${download_Url}/install/0/lib.sh && sh lib.sh $1 $2
	exit;
fi

if [ "${ubuntuVerCheck}" ];then
	debPath="u18"
elif [ "${debianVerCheck}" ];then
	debPath="d10"
fi
