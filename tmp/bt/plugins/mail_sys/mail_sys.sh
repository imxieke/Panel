...
if [ ! -f $public_file ];then
	wget -O $public_file http://download.bt.cn/install/public.sh -T 5 --no-check-certificate
fi
. $public_file
...
#    apt install rspamd -y
    ...
    wget -O- https://rspamd.com/apt-stable/gpg.key --no-check-certificate| gpg --dearmor | tee /etc/apt/keyrings/rspamd.gpg > /dev/null
...
Install()
{
  ...
  if [ "$?" -ne 0 ];then
    wget -O $pluginPath/mail_server_init.py $download_Url/install/plugin/mail_sys/mail_server_init.py -T 5 --no-check-certificate
    wget -O $pluginPath/services_file.txt $download_Url/install/plugin/mail_sys/services_file.txt -T 5 --no-check-certificate
