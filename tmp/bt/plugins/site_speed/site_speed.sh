...
Install_socket()
{
	...
	if [ ! -d /www/server/speed/socket.lua ]; then
		ln -sf /usr/local/share/lua/5.1/socket.lua /www/server/speed/socket.lua
		ln -sf /usr/local/share/lua/5.1/socket /www/server/speed/socket
		cp -a -r  /usr/local/lib/lua/5.1/socket/core.so /www/server/speed/socket/core.so
	fi
}
